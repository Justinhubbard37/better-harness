...
describe("Git history HTTP contract", () => {
  it("binds every Git read to the server-owned open workspace and redacts paths", async () => {
    ...
    const selections = [gitWorkspace.path, plainWorkspace];
    ...
    expect(refs).toMatchObject({ kind: "GitRefsSnapshotV1", repository: { currentBranch: "main" } });
    expect(log).toMatchObject({ kind: "GitLogPageV1", total: 1, commits: [{ sha: gitWorkspace.sha }] });
