...
test("links retained user inputs and exact file operations across layouts", async ({ page }, testInfo) => {
  ...
  const serverFile = page.locator('.input-file-tree button[title="packages/harness-studio/src/server/server.ts"]');
  await expect(serverFile).toHaveClass(/linked/);
  ...
  await page.locator('.input-file-tree button[title="DESIGN.md"]').click();
