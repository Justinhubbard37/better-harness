...
import { compileCanvasViewerModule, type CompiledCanvasViewerModule } from "./canvas-viewer-compile.js";
...
export function resolveCanvasRuntime(options: { sdkRoot?: string; sdkMedia?: string; cwd?: string } = {}): CanvasRuntime | undefined {
...
function runtimeFromMedia(media: string): CanvasRuntime | undefined {
...
function runtimeFromRoot(root: string): CanvasRuntime | undefined {
...
export async function prepareCanvasViewer(
  entry: ArtifactEntry,
  viewer: CanvasViewer,
  runtime: CanvasRuntime,
  moduleUrl: string,
): Promise<{ html: string; module: CompiledCanvasViewerModule }> {
  const payload = await generateViewerData(entry, viewer);
  ...
  return { html, module: await compileCanvasViewerModule(viewer.modulePath) };
}

export async function generateViewerData(entry: ArtifactEntry, viewer: CanvasViewer): Promise<Record<string, unknown>> {
...
export async function serveRuntimeFile(response: ServerResponse, path: string, contentType: string): Promise<void> {
