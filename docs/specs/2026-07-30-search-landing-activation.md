...
- SLA-AC-3: The home-page Quick Start has a stable `choose-host` anchor and six
  public host cards. Each card identifies the host-specific installation mode,
  report output, and setup action before linking to the matching installation
  tab and heading. Codex remains one public card but clearly exposes its
  Desktop and CLI surfaces; Qoder keeps its Canvas-output distinction.
...
- SLA-AC-6: English source strings and `zh-Hans` translation keys remain
  structurally aligned for the new metadata, hero, host-card, and report-proof
  content. Automated checks protect the descriptive title, host anchor, real
  hero media, six-card metadata, README routing, and absence of a universal
...
- Do not change report scoring, evidence semantics, host installation commands,
  the six-host public Quick Start set, or Pi's readiness boundary.
...
2. Enrich the existing six host cards with setup mode, output type, and a clear
   setup label; keep all links on the existing installation tab contract.
...
6. Extend focused source-contract tests, build both locales, run repository and
